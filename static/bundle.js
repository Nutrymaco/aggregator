(function () {
  ...
  function isEmpty(val) {
    return (Array.isArray(val) ? val : Object.keys(val)).length === 0
  }
  ...
  function textFieldValue(textField) {
    return Kefir.fromEvents(textField, 'input', (ev) => ev.target.value)
      .toProperty(() => textField.value)
  }
  ...
  async function handleSuggestionClick(ev) {
    const text = ev.currentTarget.dataset.value;
  ...
  function renderSuggestions({ text, items }) {
    ...
    <div class="suggestions">
      <ul class="suggestions__list">
        ${items.map((item) => `
          <li class="suggestions__item" data-value="${item}" onclick="(${handleSuggestionClick})(event)"        >
            <b>${text}</b>${item.slice(text.length)}
          </li>
        `).join('')}
      </ul>
    </div>
  ...
  const searchValue = textFieldValue(searchField);
  ...
  // Show popup
  searchField.addEventListener('focus', (ev) => searchAutocomplete.classList.add('search__autocomplete--visible'));

  // Hide popup
  searchField.addEventListener('blur', (ev) => searchAutocomplete.classList.remove('search__autocomplete--visible'));
  searchField.addEventListener('keydown', (ev) => ev.key === 'Escape' && ev.currentTarget.blur());
